...
class QueueService
{
     ...
     * 30% — первому в очереди (жёлтая ячейка). Если квалифицированного
     * пригласившего нет, 60% остаются проекту — больше никто не получает ничего.
     *
    ...
    public function distributeSplit(User $user, int $levelId, ?Deposit $deposit = null): void
    {
        ...
        if ($referrerEntry && $referrerMaxLevel >= $levelId) {
            ...
            // Trim: пригласивший не активен на этом уровне — 60% остаются
            // проекту, фиксируем упущенный бонус нулевой записью в леджере.
            LedgerEntry::create([
                'user_id' => $referrer->id,
        ...
        // 30% — ровно одна жёлтая ячейка первому в очереди.
